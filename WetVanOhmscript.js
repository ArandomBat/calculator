...
function RekenenWetOhm () {
    ...
    let ohm = document.getElementById("IDohm").valueAsNumber;
     ...
     switch(true) {
          ...
          output.textContent = `Voltage (U) = ${oplossing1.toFixed(2)} V`;
          break;

          case isNaN(ampere) :
          let oplossing2 = (voltage / ohm)
          output.textContent = `Ampère (I) = ${oplossing2.toFixed(2)} A`;
          break;

          case isNaN(ohm) :
          let oplossing3 = (voltage / ampere)
          output.textContent = `Weerstand (R) = ${oplossing3.toFixed(2)} Ω`;
          break;
          default:
            output.textContent = `Voltage: ${voltage} V, Ampère: ${ampere} A, Weerstand: ${ohm} Ω`;
